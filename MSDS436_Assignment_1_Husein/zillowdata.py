...
def main():
    ...
    zillow['price'] = zillow['price'].str.replace(",", '').str.replace('$', "")

    zillow['price'] = zillow['price'].astype('int')

    zillow = zillow.rename({'price':'price_USD'}, axis=1)

    zillow = zillow.dropna()

    zillow['floorSize'] = zillow['floorSize'].astype(int)
    ...
    awsapi.upload_csv(bucket_name =bucket_name,  key= '{o}'.format(o=object_name)+'{d}'.format(d=date)+'/'+'zillow_chicago_data.csv',  body=csv_buffer.getvalue())
